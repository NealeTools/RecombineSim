    ...
    for h=1:3
        ...
        while cmcount~=samples
            ...
            for j=1:16
                ...
                sc = 0;
                if strcmp(preweight,'N') == 1
                    smodel = ones(1,length(model)*10);
                else
                    weights = table2array(readtable(mapfile,'Delimiter','\t'));
                    weightidx = find(weights(:,1)==j);
                    weights(weights(weightidx,2)/100 < telomereL | weights(weightidx,2)/100 > telomereR,:) = [];
                    weightidx_adj = find(weights(:,1)==j);
                    weights_chr = weights(weightidx_adj,:);
                    weights_chr(:,2) = round(weights_chr(:,2)/100,0);
                    weights_chr(:,2) = weights_chr(:,2)-(telomereL);
                    weights_chr(find(weights_chr(:,2)<=0),:) = [];
                    weights_chr(find(weights_chr(:,2)>=length(model)),:) = [];
                    w_map = model;
                    for wg=1:length(weights_chr)
                        w_map(weights_chr(wg,2)) = w_map(weights_chr(wg,2))+weights_chr(wg,3);
                    end
                    w_map = moving_average(w_map,smoothing/100);
                    smodel = ones(1,length(model)*10);
                    smodel(edgeL:edgeR-1) = w_map;
                end
                for k=1:num
                    pos = edgeL:edgeR;
                    ...
                    if classrnd>(COratio/100)
                        ...
                        if failrnd>(failure/100)
                            sc = sc+1;
                            bound(sc,1:2) = [ds-resec,ds+resec];
                        else
                            continue
                        end
                    else
...
b = agg(:,2);
b(isnan(b)) = [];
b = b*((length(b)+(16*13))/13);
c = ((length(b)+(16*13))/13);
filenamePVALUE = strcat(pwd,'/',folder,'/','Simulations','/',genotype,'/',mode,'/','TrialResultsPValue.txt');
filenameFREQ = strcat(pwd,'/',folder,'/','Simulations','/',genotype,'/',mode,'/','TrialResultsFreq.txt');
dlmwrite(filenameFREQ,c,'-append');
filename = strcat(pwd,'/',folder,'/','Simulations','/',genotype,'/',mode,'/',genotype,'-',mode,'-',num2str(threshold),'bp','-',num2str(failure),'-',num2str(COratio),'-AggregateSim','.txt');
TAgg = array2table(agg,'VariableNames',{'NCOAvg' 'COAvg' 'TotalAvg'});
writetable(TAgg,'temp.txt','Delimiter','\t');
replaceinfile('NaN',' ','temp.txt',filename);
delete('temp.txt');
